...
public class Game2048Model extends Observable implements Model, Serializable {
	 ...
	 * @return
	 */
	public boolean solverMovement(int x, int y) {
		Board2048 oldBoard = new Board2048(this.board);
		if (!gameover) {
			shiftBoard(x, y);
		}
		boolean res = isBoardEquals(oldBoard.getBoard(), this.board.getBoard());
		this.board = oldBoard;
		return res;
	}
	...
	/**
	 * Sqeueeze the values, returns a newboard after removing 0 values after
	 * shifting a board
	 * 
	 * @param newboard
